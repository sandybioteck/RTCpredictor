BEGIN { $program_name="RTCpredictor"; $home=$ENV{'HOME'}; $hcatfile="$home/.$program_name/.$program_name.rc"; open(FF, "$hcatfile"); $varlib = <FF>; chomp($varlib); close FF; if($varlib eq "") { $varlib=$home; } } use lib $varlib;
...
$cut_col = 14;
$ripgrep = "rg";
$prefix_output = "tmp.RTCpredictor";
$pwd = `pwd`; chomp($pwd);
$jeach_side = 50;
...
print "Processing RTCpredictor database...\n";
